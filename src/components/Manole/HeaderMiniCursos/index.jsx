...
export default function HeaderMeusCursos( props ) { 
  ...
  const [logado, setLogado] = useState("");
   ...
   useEffect(() => { 
      if (getLocalStorage('userid') == 'undefined' || getLocalStorage('userid') == null || getLocalStorage('userid') == '') {
        setLogado("null");
      } else {
  ...
  return (
    <>
      ...
      <div className={`Mycursos ${styles.Myminicursos} `}>
        <Swiper
                          ...
                          <SwiperSlide>
                              <div className={ `${ styles.curso_min_card }` } key={index}>
                                  ...
                                  <div className={ `${ styles.padding } uk-flex uk-flex-right` } >
                                            <a data-Id={ item.curso_id } data-moddleid={item.curso_id_moodle}  onClick={ salvar_id } >Saber mais</a>
                                        </div>
                                    </div>
                          </SwiperSlide>
                      </> ) ) }
