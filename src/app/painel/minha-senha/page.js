...
export default function MeuPerfil() {
  ...
  const createSchema = Yup.object().shape({
    senha_antiga: Yup.string().required('A senha é obrigatória').min(3, 'A senha dever ter mais que 3 caracteres'),
    senha_nova: Yup.string().required('A senha é obrigatória').min(3, 'A senha dever ter mais que 3 caracteres'),
    senha_confirmar: Yup.string().required('A senha é obrigatória').oneOf([Yup.ref('senha_nova')], 'As senhas não correspondem'),
  })

  const RegisterForm = (schema) => {
    const { register, trigger, formState: { errors }, handleSubmit, control } = useForm( schema );
    return { register, trigger, formState: { errors }, handleSubmit, control  };
  }


  const forms = {
    password: RegisterForm( {mode: 'all', resolver: yupResolver(createSchema)}  ),
  }
  ...
  const ChangePassword = (props) => {
    ...
    setTimeout(() => {
      ...
      atualizarPerfilPassword
          .atualizacaoPerfilPassword(getLocalStorage("refleshToken"), data_password)
          .then((response) => {

            Swal.fire({
              icon: "success",
              title: "Formação!",
              text:  response.status == 200 ? "Senha trocada com sucesso!" : response.status + ": Não foi possível trocar a senha, tente novamente mais tarde!",
              ...
              <div className={`uk-margin-large-top`}>
                ...
                <div className={`${styles.container_form} `}>
                  <div>
                    ...
                    {forms.password.formState.errors?.senha_antiga &&   <p className={`uk-alert-danger`}> {forms.password.formState.errors?.senha_antiga?.message} </p> }
                  </div>
                  ...
                  <div>
                    ...
                    {forms.password.formState.errors?.senha_nova &&   <p className={`uk-alert-danger`}> {forms.password.formState.errors?.senha_nova?.message} </p> }
                  ...
                  <div>
                    ...
                    {forms.password.formState.errors?.senha_confirmar && <p className={`uk-alert-danger`}> {forms.password.formState.errors?.senha_confirmar?.message} </p> }
                ...
                <div className={`uk-margin`}>
                  <input type="submit"  className={`uk-button uk-button-primary uk-width-1-1 ${styles.atualizar_btn} `} value={"Cadastrar"}   />
                </div>
