...
export async function fetchCitiesFromUF(id_UF: number): Promise<{ value: any, label: any; }[]> {
  try {
    ...
    const optionsCities = response.data.map(city => ({ value: city.nome.toUpperCase(), label: city.nome.toUpperCase() }));
